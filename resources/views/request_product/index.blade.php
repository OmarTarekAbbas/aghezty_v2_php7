        ...
        <div class="box box-black">
            ...
            <div class="box-content">
                <div class="btn-toolbar pull-right">
                    <div class="btn-group">
                        {{-- <i class="btn btn-circle show-tooltip" title="" href="{{url('contact/create')}}"
                        data-original-title="Add new record"><i class="fa fa-plus"></i></i> --}}
                        <?php
                          $table_name = "contacts" ;
                        ?>
                    ...
                    <table id="example" class="table table-striped dt-responsive" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr class="table-flag-blue">
                                <th><input type="checkbox" name="selected_rows[]" class="contacts"
                                        value="{{$contact->id}}" onclick="collect_selected(this)"></th>
                                <th>{{$contact->name}}</th>
                                <td>{{$contact->email}}</td>
                                <td>{{$contact->phone}}</td>
                                ...
                                <td class="visible-md visible-lg">
                                    <div class="btn-group">
                                        <a class="btn btn-sm btn-default show-tooltip send_message" title="Send Message"
                                            data-lang="{{$contact->lang}}" data-con_id="{{$contact->id}}"
                                            data-product_id="{{$contact->product_id}}" data-toggle="modal"
                                            data-target="#myModal" data-original-title="Send Message"><i
                                                class="fa fa-envelope"></i></a>
                                        <a class="btn btn-sm btn-info show-tooltip" title="Show Product"
                                            href="{{url("product/".$contact->product_id."/edit")}}"
                                            data-original-title="Show Product" target="_blank"><i
                                                class="fa fa-forward"></i></a>
                                        @if(Auth::user()->hasRole('super_admin'))

                                        <a class="btn btn-sm btn-danger show-tooltip" title=""
                                            onclick="return confirm('Are you sure you want to delete this ?');"
                                            href="{{url('contact/'.$contact->id.'/delete')}}"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{url("unavailable")}}" id="form_data" method="post">
                    ...
                    <textarea name="subject" id="subject" rows="10" placeholder="@lang('messages.text-messages')"
