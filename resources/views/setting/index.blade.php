					...
					<div class="table-responsive">
						<table id="example" class="table table-striped dt-responsive" cellspacing="0" width="100%">
							...
							<tbody id="tablecontents">
								...
								@if ($setting->key == 'enable_delete' && \Auth::user()->email != 'super_admin@ivas.com')
								@else
								...
								<tr class="table-flag-blue">
									<th><input class="select_all_template" type="checkbox" name="selected_rows[]"
											value="{{$static_translation->id}}" class="roles"
											onclick="collect_selected(this)"></th>
									<td>{{$static_translation->key_word}}</td>
									<td>
										@if(strlen($static_translation->getBody()) < 50) <ul>
											...
											@else
											<a href="{{url('static_translation/'.$static_translation->id)}}"
												title="View Translation">View Translation</a>
											@endif
									</td>
									<td class="visible-md visible-lg">
										<div class="btn-group">
											<a class="btn btn-sm show-tooltip" title=""
												href="{{url('static_translation/'.$static_translation->id.'/edit')}}"
												data-original-title="Edit"><i class="fa fa-edit"></i></a>
											<a class="btn btn-sm btn-danger show-tooltip" title=""
												onclick="return confirm('Are you sure you want to delete this ?');"
												href="{{url('static_translation/'.$static_translation->id.'/delete')}}"
												data-original-title="Delete"><i class="fa fa-trash-o"></i></a>
										</div>
									</td>
								</tr>
